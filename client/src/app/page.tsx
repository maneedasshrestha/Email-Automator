...
export default function Home() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setStatus({ type: 'error', message: 'Failed to complete sending process.' });
      }
    } catch (error: any) {
      console.error(error);
      setStatus({
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-800 selection:bg-indigo-200 selection:text-indigo-900">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-200/40 via-white/0 to-white/0 pointer-events-none" />
      ...
      <div className="container mx-auto px-4 py-12 relative z-10">
        ...
        >
          <div className="inline-flex items-center justify-center p-4 bg-white rounded-2xl mb-4 shadow-sm border border-slate-100">
            <Mail className="text-indigo-600 w-8 h-8" />
          </div>
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-4 tracking-tight">
            Hackathon Mail Automator
          </h1>
          <p className="text-slate-500 max-w-lg mx-auto text-lg leading-relaxed">
            Streamline your hackathon communication. Upload your recipient list, draft your message, and hit send.
          </p>
        </motion.div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 xl:gap-12 items-start">
            ...
            <div className="space-y-4 xl:sticky xl:top-8 order-first xl:order-last">
              <div className="flex items-center justify-between px-1">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-100 px-2 py-1 rounded-sm">Device View</span>
              </div>
